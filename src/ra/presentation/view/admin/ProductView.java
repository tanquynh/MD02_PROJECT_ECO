...
import static ra.business.constant.Constant.ProductStatus.UNHIDE;
...
//            System.out.println("Nhập id catagory :");
            ...
            while (true) {
                ...
                if (selectedCategory != null) {
                    ...
                    product.setId(productDesign.autoId());
        ...
        if (index != -1) {
            ...
            while (!isExit) {
                ...
                if (st.isEmpty()) {
                    ...
                    Category newCategory = categoryDesign.findById(newCategoryId);
                    if (newCategory != null) {
            ...
            productDesign.save(productToEdit); // Cập nhật thông tin sản phẩm
    ...
    private void hideProduct() {
        ...
        Product product = productDesign.findById(idProduct);
        if (product == null) {
...
//        }
    ...
    private void sortByName() {
        List<Product> productList = productDesign.getSortNameProducts();
        displayProducts(productList);
    ...
    private void fitterByStatus() {
        ...
        switch (InputMethods.getInteger()) {
            case 1:
                List<Product> trueProduct = productDesign.findAll().stream().filter(Product::isProductStatus).collect(Collectors.toList());
                displayProducts(trueProduct);
                break;
            case 2:
                List<Product> falseProduct = productDesign.findAll().stream().filter(product -> !product.isProductStatus()).collect(Collectors.toList());
